const input = `bceadfg ebcdaf gdaecf fgcaeb fdbea fcdea cbda begdf afb ba | acfbeg cbfgea gebacf ab
...
const easyDigitLengthSet = new Set([2, 4, 3, 7]);

let res = 0;
for (const { patterns, output } of lines) {
  const digits = output.split(" ");
  for (const digit of digits) {
    if (easyDigitLengthSet.has(digit.length)) {
      res++;
    }
  }
}

const checkOrder = [1, 7, 4, 2, 3, 5, 0, 6, 9];
const easyDigitsByLength = new Map([
  [2, 1],
  [4, 4],
  [3, 7],
  [7, 8],
...
for (const { patterns, output } of lines) {
  const digits = output.split(" ");
  const sortedSignals = patterns
    .split(" ")
    .map(sortString)
    .sort((a, b) => a.length - b.length);

  const segmentsByChar = new Map();

  const chars1 = new Set(sortedSignals[0]);

  for (const char of sortedSignals[1]) {
    if (chars1.has(char)) {
      continue;
    }
    segmentsByChar.set(char, 0);
    break;
  }

  const chars4 = new Set();
  for (const char of sortedSignals[2]) {
    if (chars1.has(char)) {
      continue;
    }
    chars4.add(char);
  ...
  for (const signal of sortedSignals.slice(3, 6)) {
    const signalChars = new Set(signal);
    let allThere = true;
    for (char4 of chars4) {
      if (!signalChars.has(char4)) {
        segmentsByChar.set(char4, 1);
        allThere = false;
        break;
      }
    }
    if (allThere) {
      for (const char1 of chars1) {
        if (signalChars.has(char1)) {
          segmentsByChar.set(char1, 5);
        } else {
          segmentsByChar.set(char1, 2);
        }
      }
    }
  }
  ...
  for (char4 of chars4) {
    if (!segmentsByChar.has(char4)) {
      segmentsByChar.set(char4, 3);
      break;
    }
  }

  const leftoverChars = new Set();
  for (const char of "abcdefg") {
    if (segmentsByChar.has(char)) {
      continue;
    }
    leftoverChars.add(char);
  }

  outer: for (const signal of sortedSignals.slice(6, 9)) {
    const signalChars = new Set(signal);
    for (const lc of leftoverChars) {
      if (!signalChars.has(lc)) {
        segmentsByChar.set(lc, 4);
        break outer;
      }
    }
  }
  ...
  for (const lc of leftoverChars) {
    if (!segmentsByChar.has(lc)) {
      segmentsByChar.set(lc, 6);
      break;
    }
  }

  res += Number(
    digits.map((digit) => digitToNumber(digit, segmentsByChar)).join("")
  );
}

console.log(res);

function digitToNumber(digit, segmentsByChar) {
  const arr = [];
  for (const char of digit) {
    const segment = segmentsByChar.get(char);
    arr.push(segment);
  }
